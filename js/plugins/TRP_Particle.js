...
Game_Particle.prototype.reservePluginCommand = function(delay,interpreter,args,eventId){
	...
	this._reservedCommands.push([Number(delay),interpreter,args,eventId]);
};
...
Game_Particle.prototype.idWithSuffix = function(id){
	if(this._suffix && 
		id.indexOf(this._suffix)!==id.length-this._suffix.length)
	{
		id+=this._suffix;
	}
	return id;
...
ParticleSystem.setupSystemParticles = function(){
	...
	var particle = $gameScreen ? $gameScreen._particle : null;
	particle = particle||new Game_Particle();
...
ParticleSystem.prototype.update = function(scene){
    ...
    for(var i = 0; i<length; i=(i+1)|0){
    	...
    	if(!scene.isParticleTargetTypeValid(data.targetType))continue;
    	if(!data.destroy){
    		changed = true;
	...
	if(z===Z_INDEX.screen){
		...
		if(z>=0){
			...
			if(scene instanceof Scene_Map){
				this.spriteset(scene)._tilemap.addChild(container);
