 ...
 * @text オリジナルデータ読み込み
 * @desc 対象データベースで指定したオリジナルデータの読み込み処理を実行します。データファイルがないとエラーになります。
 * @default false
 * @type boolean
 *
 * @param autoImport
 ...
 *
 * ツクールMVのデータベースとExcelなどのシートデータ（以下シートファイル）との
 * 相互変換を行います。Excelファイル以外にもCSVやOpenDocument Spreadsheetなど
 * 7種類のフォーマットに対応しています。
 *
 * 編集したデータはプラグインコマンドから読み込み、書き出しが可能です。(後述)
 * 出力ファイルの対象は以下の通りです。
 *
 * ・データベース
 ...
 * 2. 1行目にはプロパティ名が出力されます。ここは編集しないでください。
 * 3. シート名にはデータ種別が出力されます。ここも編集しないでください。
 * 4. 2行目には項目の日本語名が出力されます。読み込み時は無視されます。
 * 5. 配列項目は一部除きjson文字列で出力されます。編集は非推奨です。
 * 6. 通常データ部は編集できますが、整合性のない値の入力には注意してください。
 * 7. Excel計算式は評価された結果がデータベースの値として読み込まれます。
 * 8. 書式設定や行列の設定、マクロの追加などは影響を与えません。
 * 9. データベースは読み込み時にID列でソートされます。重複はエラーになります。
 *
 * 本プラグインはテストプレー時のみ動作し、製品版では機能しません。
 *
 * ！要注意！
 ...
 *
 * 読み込み実行前にプロジェクトを保存したうえで、読み込み後はプロジェクトを
 * 開き直してください。開き直すとき、プロジェクトの保存はしないでください。
 *
 * プラグインコマンド詳細
 *  イベントコマンド「プラグインコマンド」から実行。
 *  （パラメータの間は半角スペースで区切る）
 *
 * プラグインコマンドはすべてイベントテスト（右クリック→Ctrl+R）から
 * 実行してください。
 *
 * EXPORT_DATABASE
