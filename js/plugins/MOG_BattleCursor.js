...
/*:
 * @plugindesc (v2.3 *) Adiciona flechas de indicação nos alvos selecionados.
 * @author Moghunter
 *
 * @param X-Axis
...
BattleCursor.prototype.createArrowActor = function() {
  ...
  for (var i = 0; i < $gameParty.maxBattleMembers(); i++) {
	   ...
	   if (this._arrow_name_visible) {
		   this._actor_name[i] = new Sprite(new Bitmap(180,32));
...
BattleCursor.prototype.createArrowEnemy = function() {
   for (var i = 0; i < this.enemies().length; i++) {
	   ...
	   if (this._arrow_name_visible) {
		   this._enemy_name[i] = new Sprite(new Bitmap(200,32));
...
BattleCursor.prototype.refresh_arrow_name = function(battler,sprite) {
	...
	sprite.opacity = 255;
	sprite.bitmap.clear();
	sprite.bitmap.drawText(battler.name(),0,0,200,32,"center");
};
...
BattleCursor.prototype.updateArrowCameraActor = function(sprite,target,battler,type) {
   if (Imported.MOG_BattleHud && !$gameSystem.isSideView())	 {
       ...
       this.y = 0;		   
	   var x = target.x;
	   var y = target.y + this._arrow_s[2];
...
/* added following 21 lines by nekoma. 2022/01/15 */
//==============================================================================
// Spriteset_Battle
//==============================================================================
...
(__createEnemies => {
    Spriteset_Battle.prototype.createEnemies = function() {
        __createEnemies.apply(this, arguments);
		if (!this.parent) {
			return;
		}
		let scene = this.parent;
		let sprite = scene.children.find(sprite => (sprite.children[0] instanceof BattleCursor));
		if (!sprite) {
			return;
		}
		sprite.removeChild(scene._battleCursor);
		let battleCursor = new BattleCursor(this, scene._actorSprites);
		sprite.addChild(battleCursor);
		scene._battleCursor = battleCursor;
    };
})(Spriteset_Battle.prototype.createEnemies);

/* added following 20 lines by nekoma. 2022/01/15 */
//==============================================================================
// Scene_Battle
//==============================================================================
...
(__onSkillOk => {
    Scene_Battle.prototype.onSkillOk = function() {
        __onSkillOk.apply(this, arguments);
		$gameTemp._skillOkFrame = Graphics.frameCount;
    };
})(Scene_Battle.prototype.onSkillOk);

(__onEnemyOk => {
    Scene_Battle.prototype.onEnemyOk = function() {
		if ($gameTemp._skillOkFrame == Graphics.frameCount) {
			this._enemyWindow.activate();
			return;
		}
        __onEnemyOk.apply(this, arguments);
    };
