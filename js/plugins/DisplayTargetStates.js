...
(function(){
    ...
    Window_TargetStates.prototype.refresh = function(target, x, y) {
        ...
        target._states.forEach(function(state, index) {
            if ($dataStates[state].autoRemovalTiming > 0) {
                this.drawText(target._stateTurns[state] , 1 + index * 32, 7, 32, 'right');
            }
        }, this);
    ...
    Scene_Battle.prototype.createAllWindows = function() {
        ...
        /* modified upper 3 lines to lower 6 lines by nekoma. 2022.01.15 */
        // var window = new Window_TargetStates();
        // _(this)._targetStates = window;
        // _(this)._windowLayer.addChild(window);
        _(this)._targetStates = [];
        $gameTroop.members().forEach(enemy => {
            var window = new Window_TargetStates();
            window._isWindow = false;   // added by nekoma. 2022.01.19
            _(this)._targetStates.push(window);
            _(this)._windowLayer.addChild(window);
        });
    };

    /* deleted following 22 lines by nekoma. 2022.01.15 */
    ...
    /* added following 43 lines by nekoma 2022.01.15 */
    (__update => {
        Scene_Battle.prototype.update = function() {
            ...
            let x = [];
            let y = [];
            let arrowAllTargets = !!($gameTemp._arrowAllTargets && $gameTemp._arrowAllTargets.some(arrow => arrow));
            if (_(this)._target[0].constructor === Game_Actor) {
                if (arrowAllTargets) {
                    $gameParty.battleMembers().forEach(actor => {
                       target.push(actor);
                    });
                } else {
                    target.push(this._actorWindow.actor());
                }
                target.forEach(actor => {
                    x.push(this._spriteset._actorSprites[actor.index()]._homeX + (+$dataActors[actor.actorId()].meta['dts_pos_x'] || 0));
                    y.push(this._spriteset._actorSprites[actor.index()]._homeY + (+$dataActors[actor.actorId()].meta['dts_pos_y'] || 0));
                });
            } else {
                if (arrowAllTargets) {
                    ...
                    target.push(this._enemyWindow.enemy());
                }
                target.forEach(enemy => {
                    x.push(enemy._screenX + (+$dataEnemies[enemy.enemyId()].meta['dts_pos_x'] || 0));
                    y.push(enemy._screenY + (+$dataEnemies[enemy.enemyId()].meta['dts_pos_y'] || 0));
                });
            }
            if (_(this)._target[0] !== target[0]) {
                for (let i = 0; i < target.length; i++) {
                    _(this)._targetStates[i].refresh(target[i], x[i], y[i]);
                }
                _(this)._target = target;
            }
        };
    ...
    Scene_Battle.prototype.onActorOk = function() {
        ...
        _(this)._target = null;
        /* modified upper 1 line to lower 1 line by nekoma. 2022.01.15 */
        // _(this)._targetStates.contents.clear();
        _(this)._targetStates.forEach(state => state.contents.clear());
    };
    ...
    Scene_Battle.prototype.onEnemyOk = function() {
        ...
        /* added following 3 lines by nekoma. 2022.01.19 */
