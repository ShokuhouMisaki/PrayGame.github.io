...
(function(){
    ...
    const _EQUIP_WEIGHT_LIMIT = [
        '102 + this.level * 2 + $gameVariables.value(561)',
        '102 + this.level * 2 + $gameVariables.value(561)',
        '102 + this.level * 2 + $gameVariables.value(561)',
        '102 + this.level * 2 + $gameVariables.value(561)',
        '102 + this.level * 2 + $gameVariables.value(561)',
        '102 + this.level * 2 + $gameVariables.value(561)',
        '100 + this.level * 2 + $gameVariables.value(561)',
        '100 + this.level * 2 + $gameVariables.value(561)',
        '100 + this.level * 2 + $gameVariables.value(561)',
        '100 + this.level * 2 + $gameVariables.value(561)',
        '100 + this.level * 2 + $gameVariables.value(561)',
    ...
    Window_EquipItem.prototype.drawItem = function(index) {
        ...
        if (!this._data[index]) {
            ...
            this.drawText('装备解除', rect.x + Window_Base._iconWidth + 4, rect.y, rect.width, 'left');
        }
    };
    ...
    Window_EquipStatus.prototype.refresh = function() {
        ...
        switch (this._displayMode) {
            case 0:
                ...
                this.drawText('←→ :选择项目', 0, this.lineHeight() * 3, this.contentsWidth());
                break;
            case 1:
    ...
    Window_EquipStatus.prototype.drawEquipTraits = function() {
        ...
        if (item) {
            ...
            item.traits.forEach(function(trait, index) {
                ...
                switch(trait.code) {
                    case 11:
                        text = $dataSystem.elements[trait.dataId] + '属性有效*' + percent + '%';
                        break;
                    case 12:
                        text = $dataSystem.terms.params[trait.dataId] + '弱化有效*' + percent + '%';
                        break;
                    case 13:
                        text = $dataStates[trait.dataId].name + '有效*' + percent + '%';
                        break;
                    case 14:
                        text = $dataStates[trait.dataId].name + '无效';
                        break;
                    case 21:
                    ...
                    case 22:
                        ...
                        break;
                    case 23:
                        word = ['被瞄准率', '防御效果', '恢复效果', '药的知识', 'MP消耗', 'TP积蓄', '物理伤害', '魔法伤害', '床伤害', '经验值获得'];
                    ...
                    case 34:
                        ...
                        break;
                    case 41:
                        text = $dataSystem.skillTypes[trait.dataId] + '可使用';
                        break;
                    case 42:
                        text = $dataSystem.skillTypes[trait.dataId] + '不可使用';
                        break;
                    case 43:
                        text = $dataSkills[trait.dataId].name + '可使用';
                        break;
                    case 44:
                        text = $dataSkills[trait.dataId].name + '不可使用';
                        break;
                    case 51:
                        text = $dataSystem.weaponTypes[trait.dataId] + '可装备';
                        break;
                    case 52:
                        text = $dataSystem.armorTypes[trait.dataId] + '可装备';
                        break;
                    case 53:
                        text = $dataSystem.equipTypes[trait.dataId] + '不可变更';
                        break;
                    case 54:
                        text = $dataSystem.equipTypes[trait.dataId] + '不可装备';
                        break;
                    case 55:
                        word = ['通常', '二刀流'];
                        text = '插槽类型：' + word[trait.dataId];
                        break;
                    case 61:
                        text = '追加行动(' + percent + '%' + ')';
                        break;
                    case 62:
                        word = ['自动战斗', '防御', '替身', 'TP保留'];
